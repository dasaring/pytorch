...
class AOTInductorModelContainer {
 public:
  AOTInductorModelContainer(
    ...
    buffers_[1].array = std::make_shared<std::vector<ConstantHandle>>(
        model->num_constants());
  ...
  const std::unordered_map<std::string, AtenTensorHandle> extract_constants_map(
    ...
    const auto& extract_map =
        use_inactive ? inactive().map : active().map;
 ...
 private:
  ...
  ConstantBufferSet& active() { return buffers_[active_idx_]; }
  ConstantBufferSet& inactive() { return buffers_[1 - active_idx_]; }
  const ConstantBufferSet& active() const { return buffers_[active_idx_]; }
  const ConstantBufferSet& inactive() const { return buffers_[1 - active_idx_]; }
