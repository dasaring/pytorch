...
class HelionKernel(SIMDKernel):
    ...
    def _first_output_buffer_name(self) -> str | None:
        """Return the name of the first non-removed output or inplace buffer."""
    ...
    def _buf_already_has_size(self, buf_name: str, target_size: list[int]) -> bool:
        """Check if a buffer already has the given size (no reinterpret needed)."""
        ...
        if buf is not None:
            ...
            return list(orig_size) == list(target_size)
    ...
    def _tile_loop_line(self, ndim: int, ref_param: str) -> str:
    ...
    def _tile_index(self, name: str, index: sympy.Expr) -> str:
        ...
        if out_ndim <= 1:
            if uses_reduction and self.inside_reduction:
                return f"{self._tile_var_name(out_ndim)}, {self._REDUCTION_SLICE}"
            return self._tile_var_name(out_ndim)
    ...
    def _compute_reinterprets_nd(self, pw_trees: list[Any]) -> None:
        ...
        for buf_name in self.args.input_buffers:
            ...
            if self._buf_already_has_size(buf_name, list(in_size)):
                continue
    ...
    def _compute_reinterprets_flat(self) -> None:
        ...
        for s in self._store_index.free_symbols:
            if s in self.range_tree_nodes:
                ...
                if not _is_reduction_prefix(entry.parent.prefix):
    ...
    def codegen_kernel(self) -> str:
        ...
        with buf.indent():
            ...
            if ndim <= 1:
                ...
                buf.writeline(self._tile_loop_line(1, ref_param))
            else:
                ...
                if ref_ndim < ndim and in_params:
                    ...
                    for inp in in_params:
                        ...
                        if self._get_buffer_ndim(inp_buf_name) == ndim:
                            ref_param = inp
                            break
                buf.writeline(self._tile_loop_line(ndim, ref_param))
            with buf.indent():
