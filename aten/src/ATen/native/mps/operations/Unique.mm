...
// Maps a PyTorch scalar dtype to the Metal symbol suffix used by Unique.metal.
static std::string unique_metal_type(ScalarType st) {
  switch (st) {
    ...
    case ScalarType::Bool:
      return "bool";
    default:
      TORCH_CHECK(false, "unique on MPS: unsupported dtype ", st);
  }
}
...
static std::tuple<Tensor, Tensor, Tensor> _unique_flat_mps_fast(const Tensor& self_flat,
  ...
  if (consecutive) {
    sorted_values = self_flat;
  } else {
    auto sort_result = at::sort(self_flat, /*dim=*/0, /*descending=*/false);
  ...
  const std::string type_name = unique_metal_type(in_dtype);
  MPSStream* stream = getCurrentMPSStream();
  ...
  // Step 5/6/7: allocate outputs.
  Tensor unique_values = at::empty({num_unique}, self_flat.options());
